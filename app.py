...
st.set_page_config(
    ...
    layout="wide", 
    page_icon="💸",
    initial_sidebar_state="expanded", # Mantém sempre aberta
    menu_items={'Get Help': None, 'Report a bug': None, 'About': None}
)

# --- PASSO 2: BARRA LATERAL FIXA E BLINDAGEM (CSS) ---
st.markdown("""
    <style>
    /* 1. Remove o Header inteiro (Garante fim do ícone do Git e Fork) */
    header[data-testid="stHeader"] {
        display: none !important;
    }
    
    /* 2. Fixa a barra lateral e impede que ela seja fechada */
    [data-testid="stSidebar"] {
        min-width: 250px !important;
        max-width: 250px !important;
    }
    [data-testid="stSidebarNav"] button {
        display: none !important; /* Remove o botão de fechar */
    }
    
    /* 3. Ajuste de espaçamento para o conteúdo principal */
    .block-container {
        padding-top: 2rem;
    }
...
# --- PASSO 7: GESTÃO DE TEMPLATES (NA SIDEBAR) ---
    st.markdown("---")
    st.caption("⚙️ GERENCIAR TEMPLATES")
    with st.expander("➕ Criar Novo"):
        with st.form("f_tmp", clear_on_submit=True):
            tn = st.text_input("Nome")
            tc = st.selectbox("Categoria", ["Alimentação", "Transporte", "Saúde", "Salário/Renda"])
...
# --- PASSO 8: LANÇAMENTOS E ADMINISTRAÇÃO ---
...
with col1:
    ...
    with st.form("f_add", clear_on_submit=True):
        ...
        c = st.selectbox("Categoria", ["Alimentação", "Transporte", "Contas Fixas", "Saúde", "Salário/Renda"])
