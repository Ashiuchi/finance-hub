...
# --- PASSO 2: BLINDAGEM VISUAL CIRÚRGICA (CSS) ---
# O segredo: Escondemos apenas o link do GitHub e o botão de Deploy, 
# preservando o botão (seta) que controla a barra lateral.
st.markdown("""
    <style>
    /* Esconde apenas o link/ícone do GitHub no topo */
    a[href*="github.com"], .stAppDeployButton {
        display: none !important;
    }
    /* Remove o rodapé e o menu de três pontos */
    footer {visibility: hidden;}
    #MainMenu {visibility: hidden;}
    
    /* Garante que a seta de expansão/recolhimento da sidebar continue visível */
...
if "user_email" not in st.session_state:
    ...
    with tab_log:
        e_l = st.text_input("E-mail", key="l_email_final")
        p_l = st.text_input("Senha", type="password", key="l_pass_final")
        if st.button("Entrar", key="btn_l_final"):
    ...
    with tab_reg:
        ne = st.text_input("Novo E-mail", key="r_email_final")
        np = st.text_input("Nova Senha", type="password", key="r_pass_final")
        if st.button("Criar Conta", key="btn_r_final"):
...
# --- PASSO 7: GESTÃO DE TEMPLATES (CRIAR E DELETAR) ---
    ...
    try:
        ...
        if tmp_data:
            st.markdown("---")
            t_del = st.selectbox("Remover Template:", [t['template_name'] for t in tmp_data], key="sel_del_t")
