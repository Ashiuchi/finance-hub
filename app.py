...
# Esconde o ícone do GitHub e o botão Deploy sem quebrar a seta da sidebar.
st.markdown("""
    <style>
    /* Alvo: Links que levam ao GitHub e botões de Deploy */
    a[href*="github.com"], .stAppDeployButton {
        display: none !important;
    }
    footer {visibility: hidden;}
    /* Garante que a seta de abrir a barra lateral continue visível */
...
if "user_email" not in st.session_state:
    ...
    with t2:
        ...
        np = st.text_input("Senha (mín. 6 chars)", type="password", key="r_pass")
...
# --- PASSO 7: GESTÃO DE TEMPLATES (CRIAR/DELETAR) ---
    ...
    with st.expander("➕ Novo Template"):
        with st.form("new_tmp_f", clear_on_submit=True):
            ...
            tc = st.selectbox("Categoria", ["Alimentação", "Transporte", "Certificações", "Salário/Renda"])
